...
def parse_args():
    ...
    print("="*50)
    print(f"[DEBUG] config_args: {config_args}")
    print(f"[DEBUG] config: {config_args.config}")
    print(f"[DEBUG] config_file: {config_args.config_file}")
    print("="*50)
    ...
    args.eval_batch_size = args.batch_size # FIXME: to avoid OOM
...
def main():
    ...
    with utils.distributed.sync_workers() as rank:
        if rank == 0:
            print(f"[DEBUG] working directory: {args.work_dir}")
            create_exp_dir(args.work_dir,
                           scripts_to_save=['src/train_DTP.py', 'src/hourglass.py'],)
