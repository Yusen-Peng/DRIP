...
from ..llava_arch import LlavaMetaModel, LlavaMetaForCausalLM, unpad_image, get_anyres_image_grid_shape
from ..modeling_llama_pdrop import LlamaModel, LlamaForCausalLM
...
class LlavaLlamaForCausalLM_PDrop(LlavaLlamaForCausalLM):
    def prepare_inputs_labels_for_multimodal_pdrop(
            ...
            for i in range(num_images + 1):    # 将原本text以<image>为界线分开，分别embed，并且append image feature
                ...
                if i < num_images:
                    ...
                    if cur_image_features.dim() == 3:
                        assert cur_image_features.shape[0] == 1
                        cur_image_features = cur_image_features.squeeze(0)
                    # Record ACTUAL number of image tokens inserted
                    image_token_lengths.append(cur_image_features.shape[0])
                        
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))

            cur_new_input_embeds = [x.to(self.device) for x in cur_new_input_embeds]
            # for i, x in enumerate(cur_new_input_embeds):
            #     print(
            #         f"[DEBUG] cur_new_input_embeds[{i}] "
            #         f"shape={x.shape}, ndim={x.ndim}"
            #     )
        ...
        self.model.image_tokens = image_token_lengths
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            (
                ...
                inputs_embeds,
                labels,
                _,  # boundary_loss, unused during inference
            ) = self.prepare_inputs_labels_for_multimodal_pdrop(
                input_ids,
                position_ids,
                attention_mask,
                past_key_values,
        ...
        outputs,labels = self.model.pdrop_forward(  
                        input_ids=input_ids,
                        attention_mask=attention_mask,
                        position_ids=position_ids,
                        past_key_values=past_key_values,
        ...
        **kwargs,
        ...
        if images is not None:
            (
                ...
                inputs_embeds,
                _,
                _,  # additional unused return value
            ) = self.prepare_inputs_labels_for_multimodal_pdrop(
                inputs,
                position_ids,
                attention_mask,
...
AutoModelForCausalLM.register(LlavaConfig, LlavaLlamaForCausalLM_PDrop)