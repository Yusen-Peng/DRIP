    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images and cur_image_idx < len(image_features): # fix index error
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
...
"""
    ...
    https://github.com/ZhengyaoFang/PruneSID/blob/main/prunesid/prunesid_llava/llava_arch.py
"""

def encode_images_prunesid(self, images):
    ...
    projector_device = next(self.get_model().mm_projector.parameters()).device
...
def encode_images_prunesid_multi(self, images):
    image_features, keep_idx = self.get_model().get_vision_tower().forward(images)
    projector_dtype = next(self.get_model().mm_projector.parameters()).dtype
    projector_device = next(self.get_model().mm_projector.parameters()).device
    image_features = image_features.to(device=projector_device, dtype=projector_dtype)
...
def restore_image_features_sorted(self, image_feature, cur_keep_idx, width, height):
    ...
    num_img = len(image_feature)
    restored_features = []
    masks = []
    cls_img_feature_merge = []
    raw_img_feature_merge = []
    for i in range(num_img):
        total_patches, feature_dim = image_feature[i].shape
        num_keep = torch.where(cur_keep_idx[i] != -1)[0].shape[0]
        cls_img_feature_merge.append(image_feature[i][0].unsqueeze(0))
        raw_img_feature_merge.append(image_feature[i][num_keep:,])
        patch_keep_idx = cur_keep_idx[i][1:num_keep] - 1
        patch_keep_idx = patch_keep_idx.sort(dim=0)[0]
        restore_feature = torch.zeros((576, feature_dim), device=image_feature[0].device, dtype=image_feature[0].dtype)
        mask = torch.zeros(576, dtype=torch.bool, device=image_feature[0].device)
        mask.scatter_(0, patch_keep_idx, True)
        masks.append(mask)
        kept_features = image_feature[i][1:num_keep,:]
        restore_feature[mask] = kept_features.reshape(-1, feature_dim)
        restored_features.append(restore_feature)

    restored_features = torch.stack(restored_features)
    ...
    image_feature_select = grid_with_newline[mask_all] # [322,4096] 322 = 161*2
    raw_img_feature_merge = torch.cat(raw_img_feature_merge, dim=0)
    cls_img_feature_merge = torch.cat(cls_img_feature_merge, dim=0)

    image_feature_select = torch.cat([image_feature_select, cls_img_feature_merge, raw_img_feature_merge])
    return image_feature_select #[384,4096]

    
def prepare_inputs_labels_for_multimodal_prunesid(
    self, input_ids, position_ids, attention_mask, past_key_values, labels,
    images, image_sizes=None, inference: bool = False
):
    vision_tower = self.get_vision_tower()
    ...
    if type(images) is list or images.ndim == 5:
        ...
        image_features = self.encode_images_prunesid(images)
