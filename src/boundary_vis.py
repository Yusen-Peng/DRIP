...
def visualize_boundaries_enhanced(model: DTPViT, image_tensor: torch.Tensor, save_path=None):
    """
    Visualizes learned boundaries with red-tinted overlays on patches (2D) and linearized patch strip.
    ...
    x = x + model.pos_embed[:, :x.size(1), :]
    x = model.pos_drop(x)
    x = model.pre_blocks(x.transpose(0, 1))  # [1+N, B, D]

    # Predict boundaries
    _, hard_boundaries = model.boundary_predictor(x)
    hard_boundaries = hard_boundaries[:, 1:]  # remove CLS
    grid_h, grid_w = model.patch_embed.grid_size
    boundary_mask = hard_boundaries[0].cpu().view(grid_h, grid_w).numpy()
    ...
    for i in range(grid_h):
        for j in range(grid_w):
            if boundary_mask[i, j] > 0.5:
                ...
                blended = (0.6 * patch + 0.4 * red_overlay).astype(np.uint8)
                overlay_np[y0:y1, x0:x1] = blended
    ...
    for i in range(grid_h):
        for j in range(grid_w):
            ...
            if boundary_mask[i, j] > 0.5:
                red_overlay = np.zeros_like(patch)
                red_overlay[..., 0] = 255
                patch = (0.6 * patch + 0.4 * red_overlay).astype(np.uint8)
            patch_list.append(Image.fromarray(patch))

    num_cols = len(patch_list)
    raster_strip = Image.new("RGB", (patch_w * num_cols, patch_h))
    for idx, patch in enumerate(patch_list):
        raster_strip.paste(patch, (idx * patch_w, 0))

    # === Plot results: 2D side-by-side + 1D raster below ===
    fig = plt.figure(figsize=(max(10, num_cols * patch_w / 60), 10))

    # Top row: Original and Boundary Overlay side-by-side
    ...
    ax2.set_title("Boundaries (Red Overlay)")
    ax2.axis('off')

    # Bottom row: 1D Rasterized Patches
    ax3 = plt.subplot2grid((2, 2), (1, 0), colspan=2)
    ax3.imshow(raster_strip)
    ax3.set_title("1D Rasterized Patches (Red = Boundary)")
    ax3.axis('off')
