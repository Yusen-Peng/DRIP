...
# https://github.com/Yuliang-Liu/MultimodalOCR/tree/main/OCRBench
...
def eval_worker(args, data, eval_id, output_queue):
    try:
        ...
        for i in tqdm(range(len(data))):
            ...
            # Some generate() implementations return full sequence;
            # others return only newly generated tokens.
...
if __name__=="__main__":
    ...
    pool = Pool(processes=args.num_workers)
    for i in range(len(data_list)):
        r = pool.apply_async(eval_worker, args=(args, data_list[i], i, output_queue))
        async_results.append(r)
    pool.close()
    pool.join()

    # reveal hidden worker crash
    ...
    if len(data)==1000:
        ...
        print(f"Accuracy: {Final_score/10:.4f}%")
    else:
        for i in range(len(data)):
