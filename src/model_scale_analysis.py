...
def compute_drip_gain(df):
    """
    Compute DRIP - Fixed Pooling for each compression level.
...
# Zero = DRIP and Fixed are equivalent
ax.axhline(
    0,
    linewidth=0.8,
    linestyle="--",
...
ax.set_ylabel("DRIP − Fixed (normalized pts.)")
...
for level, fixed, drip in zip(
    ...
    print(
        ...
        f"Fixed={fixed:+.2f}, "
        f"DRIP={drip:+.2f}, "
        f"DRIP advantage={drip-fixed:+.2f}"
    )
...
plt.savefig(
    "results/llm_scaling_gain_ocr.png",
    bbox_inches="tight",
    dpi=300,
)
plt.show()