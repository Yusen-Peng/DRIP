...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to the JSON or LLaVA JSONL prediction file."
    ...
    parser.add_argument(
        ...
        help="Path to prepared ChartQAPro GT JSON. Required for LLaVA JSONL."
    ...
    # If LLaVA jsonl, merge with GT file into original evaluator format

    if args.predictions_file.endswith(".jsonl"):

        assert args.gt_file is not None, "--gt-file is required for LLaVA jsonl predictions"

        gt = json.load(open(args.gt_file, "r", encoding="utf-8"))

        pred_map = {str(x["question_id"]): x["text"] for x in predictions}
        ...
        for item in gt:
            ...
            item["prediction"] = pred_map.get(qid, "")
