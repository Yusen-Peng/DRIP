...
from open_clip_local.DTP_ViT import DTPViT_Causal, DTPViT_CosSim
...
@torch.no_grad()
def get_causal_hard_boundaries(model: DTPViT_Causal, img_3chw: torch.Tensor):
    """
    For DTPViT_Causal:
      input image -> _embeds -> causal transformer_pre -> boundary_predictor
    ...
    # pre transformer with causal mask
    hidden_states = model.transformer_pre(hidden_states, attn_mask=pre_mask)
...
def overlay_causal_boundaries(
    ...
    Returns:
      PIL.Image with boundary-kept patches overlaid in red.
    """
    hard_1d, grid_h, grid_w = get_causal_hard_boundaries(model, img_3chw)
...
@torch.no_grad()
def visualize_boundaries_single_multi_causal(
    model: DTPViT_Causal,
    preprocess,
    root_dir="/users/PAS2912/yusenpeng/Fast-CLIP/src/boundary_vis/image_samples/single_multi/",
    save_path="/users/PAS2912/yusenpeng/Fast-CLIP/src/boundary_vis/single_multi_overlay_dtpvit.png",
    ...
    overlays = [
        overlay_causal_boundaries(model, t, mean=mean, std=std)
        for t in tensors
    ]
...
def main():
    ...
    #ckpt_path = "/fs/scratch/PAS2836/yusenpeng_checkpoint/imagenet_DRIP_temp_01/model_19.pth"
    #ckpt_path = "/fs/scratch/PAS2836/yusenpeng_checkpoint/imagenet_DRIP_temp_005/model_19.pth"
    ckpt_path = "/fs/scratch/PAS2836/yusenpeng_checkpoint/imagenet_DRIP_no_sample/model_19.pth"
    ...
    if TYPE == "bidirectional" or TYPE == "H-Net":
        ...
        visualize_boundaries_single_multi_causal(
            model,
            preprocess,
            root_dir="/users/PAS2912/yusenpeng/Fast-CLIP/src/boundary_vis/image_samples/single_multi/",
