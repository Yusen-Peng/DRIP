...
def eval_worker(args, data, eval_id, output_queue):
    try:
        print(f"[worker {eval_id}] start, samples={len(data)}")
        ...
        if (
            ...
            conv_mode += "_mmtag"
            print(f"[worker {eval_id}] auto switch conv_mode -> {conv_mode}")

        output_data = []

        for i in tqdm(range(len(data)), desc=f"worker-{eval_id}"):

            row = data[i]

            if row.get("predict", None) not in [None, "", 0]:
                output_data.append(normalize_row(row))
                continue
        ...
        output_queue.put({eval_id: output_data})
        print(f"[worker {eval_id}] completed")

    except Exception:
        print(f"🔥 worker {eval_id} crashed")
        traceback.print_exc()
        raise
