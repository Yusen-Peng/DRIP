...
def segments_to_matrix_diff_detached_cumsum(
    ...
    if leading_one:
        ...
        n_segments = int(
            (boundaries.detach() > 0.5)
            .sum(dim=-1)
            .max()
            .item()
        )

        cs = detached_cumsum(boundaries, dim=-1)
        cs = cs.unsqueeze(-1)

        js = (
            torch.zeros_like(boundaries).unsqueeze(2)
            + torch.arange(
                n_segments,
        ...
        mat = mat / (
            mat.sum(dim=1, keepdim=True) + 1e-9
        ).detach()
...
def new_downsample(boundaries: torch.Tensor, hidden: torch.Tensor, null_group: torch.Tensor, leading_one: bool = False):
    ...
    _, _, D = hidden.shape
    # NOTE: Integer overflow bug is now fixed:
    # Count discrete boundaries in integer precision.
    seg_counts = (boundaries.detach() > 0.5).sum(dim=1)
