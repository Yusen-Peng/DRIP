...
class CompressedQwen3VLVisionPatchMerger(Qwen3VLVisionPatchMerger):
    def __init__(self, config: Qwen3VLVisionConfig, use_postshuffle_norm: bool = False):
        super().__init__(config=config, use_postshuffle_norm=use_postshuffle_norm)
        self.compressor = None
        # Qwen spatial merge is normally 2x2.
        self.spatial_merge_size = config.spatial_merge_size
        self.merge_area = self.spatial_merge_size ** 2
        # self.hidden_size is the mega-token dimension.
        # e.g. 1024 = 4096 // 4
        self.bp_hidden_size = self.hidden_size // self.merge_area
    ...
    def forward(self, x: torch.Tensor, inference: bool = False):
        ...
        if self.compressor is not None:
            ...
            if self.compressor.merge_strategy == "DRIP":
                boundaries = (
            ...
            elif self.compressor.merge_strategy == "Fixed":
                boundaries = (
                    self.compressor.get_fixed_pooled_boundaries(
                        patch_features
                    )
                )
            else:
                raise ValueError(f"Unknown merge strategy:{self.compressor.merge_strategy}")
            x = x.unsqueeze(0)
...
class CompressedQwen3VLModel(Qwen3VLModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        ...
        # we need to unpack boundaries and boundary loss
        (
            image_embeds,
        ...
        if image_grid_thw.shape[0] != 1:
            raise NotImplementedError("Initial compressed implementation supports exactly one image.")
        image_embeds = (image_embeds,)
        return image_embeds, deepstack_image_embeds, boundaries, boundary_loss

    def compress_deepstack_features(self, deepstack_image_embeds, boundaries):
        compressor = self.visual.merger.compressor
        if compressor is None:
            raise RuntimeError("Compressor has not been initialized. Call model.set_compressor() first.")
        ...
        for deep in deepstack_image_embeds:
            ...
            deep = deep.unsqueeze(0)  # [1, L, D_llm]
            compressed_deep = compressor.apply_boundaries(deep, boundaries)
            compressed_deepstack.append(compressed_deep.squeeze(0))
        return compressed_deepstack
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            compressed_image_embeds = image_embeds[0]
            if input_ids is None:
                raise NotImplementedError("Compressed image path currently requires input_ids.")
        ...
        keep_mask = None
        if pixel_values is not None:
            compressed_deepstack = self.compress_deepstack_features(deepstack_image_embeds, boundaries)
            ...
            if (cache_position is not None and cache_position.ndim == 1 and cache_position.numel() == keep.numel()):
                cache_position = torch.arange(int(keep.sum().item()), device=cache_position.device, dtype=cache_position.dtype)
...
class CompressedTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial):
        ...
        drip_keys_to_match = ["boundary_predictor"]
